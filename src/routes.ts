...
router.post("/post", async (req: Request, res: Response) => {
  try {
    ...
    // Emit a codeUpdate event to notify clients about the new code
    if (io) {
      io.then((socket) => {
        socket?.emit("codeUpdate", newCodeData);
      });
    }
...
router.put("/update", async (req: Request, res: Response) => {
  try {
    ...
    // Emit a codeUpdate event to notify clients about the updated code
    if (io) {
      io.then((socket) => {
        socket?.emit("codeUpdate", updatedCodeData);
      });
    }
